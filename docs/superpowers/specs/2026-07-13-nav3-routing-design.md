...
- `org.jetbrains.compose.material3.adaptive:adaptive-navigation3:1.3.0-beta02`
  — адаптивный list-detail (см. ниже). Beta.
...
### Адаптивный list-detail

На широких экранах (планшет, разложенный фолдабл, десктоп) список заметок и
открытая заметка показываются рядом; на узких — обычный бэкстек (список → пуш
заметки → back). Реализуется `ListDetailSceneStrategy` из `adaptive-navigation3`
поверх того же `NavDisplay` — маршрутам через metadata назначается роль пейна:

- `VaultList` → list-пейн
- `Note` → detail-пейн

Ключевые следствия для дизайна:

- Пуш `VaultList → Note` остаётся тем же кодом. Разбивать на две колонки или
  показывать в одну решает стратегия по ширине окна, а не мы вручную. Логика
  «назад» и «заметка из AI» не меняется.
- **Пустой detail-пейн.** На широком экране при выбранной папке, но без
  открытой заметки, detail показывает заглушку («выберите заметку»). Добавить
  простой placeholder-composable.
- **Папки.** `VaultList(dir)` — тоже list-пейн; вложенная папка заменяет
  содержимое list-пейна (пуш), detail при этом не сбрасываем без нужды —
  поведение уточняется при реализации, дефолт: смена папки закрывает detail.
- Онбординг, `Settings`, `ModelPicker`, `AiChat` — **не** участвуют в
  list-detail, показываются на всю ширину (обычная сцена).
- `WindowSizeClass` для порога берём из material3-adaptive; ничего своего.
...
- iOS/desktop/web таргеты — конфиг сериализации делаем корректным, но таргеты
  не добавляем. (list-detail заработает и на Android — большие экраны/фолдаблы.)
- Диплинки.
...
2. `adaptive-navigation3` в beta (1.3.0-beta02) — API list-detail может
   поменяться; допустимо, экран заметок от этого функционально не страдает.
3. Nav3 KMP относительно новый — если всплывёт блокер на 1.1.1, фолбэк —
