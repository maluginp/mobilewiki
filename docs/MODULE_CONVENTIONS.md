...
- Реализация репозитория скрыта за `internal OkioVaultRepository`; создаётся только в `di` (androidMain `vaultPlatformModule`).
- `di/vaultModule` (commonMain) байндит `VaultViewModel`; `vaultPlatformModule` (androidMain) — `VaultRepository` + IO-диспетчер. Подключён в `BrainerApp`.
